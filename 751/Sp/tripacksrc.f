      SUBROUTINE ADDCST_ext (NCC,LCC,N,X,Y, LWK,IWK,LIST,LPTR,
     .                   LEND, IER)
      dll_export ADDCST_ext
      INTEGER NCC, LCC(*), N, LWK, IWK(LWK), LIST(*),
     .        LPTR(*), LEND(N), IER
      REAL    X(N), Y(N)
      call ADDCST (NCC,LCC,N,X,Y, LWK,IWK,LIST,LPTR,
     .                   LEND, IER,2)
      return
      end
...
C
   10 IER = 5
      RETURN
      END
      SUBROUTINE ADDNOD_ext (K,XK,YK,IST,NCC, LCC,N,X,Y,LIST,
     .                   LPTR,LEND,LNEW, IER)
      dll_export ADDNOD_ext
      INTEGER K, IST, NCC, LCC(*), N, LIST(*), LPTR(*),
     .        LEND(*), LNEW, IER
      REAL    XK, YK, X(*), Y(*)
      call ADDNOD (K,XK,YK,IST,NCC, LCC,N,X,Y,LIST,
     .                   LPTR,LEND,LNEW, IER)
      RETURN
      END
...
C
   11 IER = -4
      RETURN
      END
      REAL FUNCTION AREAP_ext (X,Y,NB,NODES)
      dll_export AREAP_ext
      INTEGER NB, NODES(NB)
      REAL    X(*), Y(*)
      AREAP_ext = AREAP (X,Y,NB,NODES)
      RETURN
      END
...
C
    2 AREAP = -A/2.
      RETURN
      END
      SUBROUTINE BDYADD_ext (KK,I1,I2, LIST,LPTR,LEND,LNEW )
      dll_export BDYADD_ext
...
C
    4 LIST(LNEW) = -N2
      ...
      RETURN
      END
      SUBROUTINE BNODES_ext (N,LIST,LPTR,LEND, NODES,NB,NA,NT)
      dll_export BNODES_ext
...
C
    4 NB = K
      ...
      RETURN
      END
      SUBROUTINE CIRCUM_ext (X1,Y1,X2,Y2,X3,Y3,RATIO, XC,YC,CR,
     .                   SA,AR)
      dll_export CIRCUM_ext
      LOGICAL RATIO
      REAL    X1, Y1, X2, Y2, X3, Y3, XC, YC, CR, SA, AR
      call CIRCUM (X1,Y1,X2,Y2,X3,Y3,RATIO, XC,YC,CR,
     .                   SA,AR)
      RETURN
      END
...
C
    ...
    2   CONTINUE
     ...
     .     ( (SQRT(DS(1)) + SQRT(DS(2)) + SQRT(DS(3)))*CR )
      RETURN
      END
      LOGICAL FUNCTION CRTRI_ext (NCC,LCC,I1,I2,I3)
      dll_export CRTRI_ext
      INTEGER NCC, LCC(*), I1, I2, I3
      logical CRTRI
      CRTRI_ext = CRTRI (NCC,LCC,I1,I2,I3)
      RETURN
      END
...
C
    2 CRTRI = .FALSE.
      RETURN
      END
      SUBROUTINE DELARC_ext (N,IO1,IO2, LIST,LPTR,LEND,
     .                   LNEW, IER)
      dll_export DELARC_ext
      INTEGER N, IO1, IO2, LIST(*), LPTR(*), LEND(N), LNEW,
     .        IER
      call DELARC (N,IO1,IO2, LIST,LPTR,LEND,
     .                   LNEW, IER)
      RETURN
      END
...
C
      ...
      RETURN
      END
      SUBROUTINE DELNB_ext (N0,NB,N, LIST,LPTR,LEND,LNEW, LPH)
      dll_export DELNB_ext
      INTEGER N0, NB, N, LIST(*), LPTR(*), LEND(N), LNEW,
     .        LPH
      call DELNB (N0,NB,N, LIST,LPTR,LEND,LNEW, LPH)
      RETURN
      END
...
C
      ...
      RETURN
      END
      SUBROUTINE DELNOD_ext (K,NCC, LCC,N,X,Y,LIST,LPTR,LEND,
     .                   LNEW,LWK,IWK, IER)
      dll_export DELNOD_ext
      INTEGER K, NCC, LCC(*), N, LIST(*), LPTR(*),
     .        LEND(*), LNEW, LWK, IWK(2,*), IER
      REAL    X(*), Y(*)
      call DELNOD (K,NCC, LCC,N,X,Y,LIST,LPTR,LEND,
     .                   LNEW,LWK,IWK, IER,2)
      RETURN
      END
...
!     .        ', IER = ',I1,' ***'/)
      END
      SUBROUTINE EDGE_ext (IN1,IN2,X,Y, LWK,IWK,LIST,LPTR,
     .                 LEND, IER)
      dll_export EDGE_ext
      INTEGER IN1, IN2, LWK, IWK(2,*), LIST(*), LPTR(*),
     .        LEND(*), IER
      REAL    X(*), Y(*)
      call EDGE (IN1,IN2,X,Y, LWK,IWK,LIST,LPTR,
     .                 LEND, IER,2)
      return
      END
...
!     .        ', IER = ',I1,' ***'/)
      RETURN
      END
      SUBROUTINE GETNP_ext (NCC,LCC,N,X,Y,LIST,LPTR,LEND,
     .                  L, NPTS,DS, IER)
      dll_export GETNP_ext
...
C
   ...
   17   CONTINUE
      ...
      RETURN
      END
      INTEGER FUNCTION INDXCC_ext (NCC,LCC,N,LIST,LEND)
      dll_export INDXCC_ext
      INTEGER NCC, LCC(*), N, LIST(*), LEND(N)
      INDXCC_ext = INDXCC (NCC,LCC,N,LIST,LEND)
      RETURN
      END
...
C
    5 INDXCC = I
      RETURN
      END
      SUBROUTINE INSERT_ext (K,LP, LIST,LPTR,LNEW )
      dll_export INSERT_ext
...
C
      ...
      RETURN
      END
      SUBROUTINE INTADD_ext (KK,I1,I2,I3, LIST,LPTR,LEND,LNEW )
      dll_export INTADD_ext
...
C
      ...
      RETURN
      END
      LOGICAL FUNCTION INTSEC_ext (X1,Y1,X2,Y2,X3,Y3,X4,Y4)
      dll_export INTSEC_ext
      REAL X1, Y1, X2, Y2, X3, Y3, X4, Y4
      logical INTSEC
      INTSEC_ext = INTSEC (X1,Y1,X2,Y2,X3,Y3,X4,Y4)
      RETURN
      END
...
C
    1 INTSEC = A .EQ. 0.  .AND.  B .EQ. 0.
      RETURN
      END
      INTEGER FUNCTION JRAND_ext (N, IX,IY,IZ )
      dll_export JRAND_ext
      INTEGER N, IX, IY, IZ
      JRAND_ext = JRAND (N, IX,IY,IZ )
      RETURN
      END
...
C
     ...
     .    (REAL(IZ)/30323.)
      ...
      RETURN
      END
      LOGICAL FUNCTION LEFT_ext (X1,Y1,X2,Y2,X0,Y0)
      dll_export LEFT_ext
      REAL    X1, Y1, X2, Y2, X0, Y0
      Logical LEFT
      LEFT_ext = LEFT (X1,Y1,X2,Y2,X0,Y0)
      RETURN
      END
...
C
      ...
      RETURN
      END
      INTEGER FUNCTION LSTPTR_ext (LPL,NB,LIST,LPTR)
      dll_export LSTPTR_ext
      INTEGER LPL, NB, LIST(*), LPTR(*)
      LSTPTR_ext = LSTPTR (LPL,NB,LIST,LPTR)
      RETURN
      END
...
C
    2 LSTPTR = LP
      RETURN
      END
      INTEGER FUNCTION NBCNT_ext (LPL,LPTR)
      dll_export NBCNT_ext
      INTEGER LPL, LPTR(*)
      NBCNT_ext = NBCNT (LPL,LPTR)
      RETURN
      END
...
C
    2 NBCNT = K
      RETURN
      END
      INTEGER FUNCTION NEARND_ext (XP,YP,IST,N,X,Y,LIST,LPTR,
     .                         LEND, DSQ)
      dll_export NEARND_ext
      INTEGER IST, N, LIST(*), LPTR(*), LEND(N)
      REAL    XP, YP, X(N), Y(N), DSQ
      NEARND_ext =NEARND (XP,YP,IST,N,X,Y,LIST,LPTR,
     .                         LEND, DSQ)
      RETURN
      END
...
C
    7 NEARND = 0
      RETURN
      END
      SUBROUTINE OPTIM_ext (X,Y,NA, LIST,LPTR,LEND,NIT,IWK, IER)
      dll_export OPTIM_ext
      INTEGER NA, LIST(*), LPTR(*), LEND(*), NIT, IWK(2,NA),
     .        IER
      REAL    X(*), Y(*)
      call OPTIM (X,Y,NA, LIST,LPTR,LEND,NIT,IWK, IER)
      RETURN
      END
...
C
    9 NIT = ITER
      ...
      RETURN
      END
      REAL FUNCTION STORE_ext (X)
      dll_export STORE_ext
      REAL X
      STORE_ext = STORE (X)
      RETURN
      END
...
C
      ...
      RETURN
      END
      SUBROUTINE SWAP_ext (IN1,IN2,IO1,IO2, LIST,LPTR,
     .                 LEND, LP21)
      dll_export SWAP_ext
...
C
      ...
      RETURN
      END
      LOGICAL FUNCTION SWPTST_ext (IN1,IN2,IO1,IO2,X,Y)
      dll_export  SWPTST_ext
      INTEGER IN1, IN2, IO1, IO2
      REAL    X(*), Y(*)
      Logical SWPTST
      SWPTST_ext = SWPTST (IN1,IN2,IO1,IO2,X,Y)
      RETURN
      END
...
C
    2 SWPTST = .FALSE.
      RETURN
      END
      SUBROUTINE TRFIND_ext (NST,PX,PY,N,X,Y,LIST,LPTR,LEND, I1,
     .                   I2,I3)
      dll_export TRFIND_ext
      INTEGER NST, N, LIST(*), LPTR(*), LEND(N), I1, I2, I3
      REAL    PX, PY, X(N), Y(N)
      call TRFIND (NST,PX,PY,N,X,Y,LIST,LPTR,LEND, I1,
     .                   I2,I3)
      RETURN
      END
...
C
   17 I1 = 0
      ...
      RETURN
      END
      SUBROUTINE TRLIST_ext (NCC,LCC,N,LIST,LPTR,LEND,NROW, NT,
     .                   LTRI,LCT,IER)
      dll_export TRLIST_ext
      INTEGER NCC, LCC(*), N, LIST(*), LPTR(*), LEND(N),
     .        NROW, NT, LTRI(NROW,*), LCT(*), IER
      call TRLIST (NCC,LCC,N,LIST,LPTR,LEND,NROW, NT,
     .                   LTRI,LCT,IER)
      RETURN
      END
...
C
   13 NT = 0
      ...
      RETURN
      END
      SUBROUTINE TRLPRT_ext (NCC,LCT,N,X,Y,NROW,NT,LTRI,LOUT,
     .                   PRNTX)
      dll_export TRLPRT_ext
      INTEGER NCC, LCT(*), N, NROW, NT, LTRI(NROW,NT),
     .        LOUT
      LOGICAL PRNTX
      REAL    X(N), Y(N)
      call TRLPRT (NCC,LCT,N,X,Y,NROW,NT,LTRI,LOUT,
     .                   PRNTX,2)
      RETURN
      END
...
!     .        ', NROW =',I5,', NT =',I5,' ***')
      END
      SUBROUTINE TRMESH_ext (N,X,Y, LIST,LPTR,LEND,LNEW,NEAR,
     .                   NEXT,DIST,IER)
      dll_export TRMESH_ext
      INTEGER N, LIST(*), LPTR(*), LEND(N), LNEW, NEAR(N),
     .        NEXT(N), IER
      REAL    X(N), Y(N), DIST(N)
      call TRMESH (N,X,Y, LIST,LPTR,LEND,LNEW,NEAR,
     .                   NEXT,DIST,IER)
      return
      end
...
C
    ...
    7   CONTINUE
      RETURN
      END
      subroutine StoreSwtol_ext(value)
      dll_export StoreSwtol_ext
      REAL    value, SWTOL
      COMMON/SWPCOM/SWTOL
      swtol = value
      return
...
C
   12 IER = K
      RETURN
      END
      SUBROUTINE TRPLOT_ext (LUN,PLTSIZ,WX1,WX2,WY1,WY2,NCC,LCC,
     .                   N,X,Y,LIST,LPTR,LEND,
     .                   NUMBR, IER)
      dll_export TRPLOT_ext
      CHARACTER(80) TITLE
      INTEGER LUN, NCC, LCC(*), N, LIST(*), LPTR(*),
     .        LEND(N), IER
      LOGICAL NUMBR
      REAL    PLTSIZ, WX1, WX2, WY1, WY2, X(N), Y(N)
        call TRPLOT (LUN,PLTSIZ,WX1,WX2,WY1,WY2,NCC,LCC,
     .                   N,X,Y,LIST,LPTR,LEND,'dummy',
     .                   NUMBR, IER,4)
      RETURN
      END
...
C
   13 IER = 3
      RETURN
      END
      SUBROUTINE TRPRNT_ext (NCC,LCC,N,X,Y,LIST,LPTR,LEND,LOUT,
     .                   PRNTX)
      dll_export TRPRNT_ext
      INTEGER NCC, LCC(*), N, LIST(*), LPTR(*), LEND(N),
     .        LOUT
      LOGICAL PRNTX
      REAL    X(N), Y(N)
      call TRPRNT (NCC,LCC,N,X,Y,LIST,LPTR,LEND,LOUT,
     .                   PRNTX,2)
      RETURN
      END
