...
C
      PRNTX = .TRUE.
      CALL TRPRNT (NCC,LCC,N,X,Y,LIST,LPTR,LEND,LOUT,PRNTX,ifortran)
      CALL TRLIST (NCC,LCC,N,LIST,LPTR,LEND,NROW, NT,LTRI,
     .             LCT,IER,ifortran)
      CALL TRLPRT (NCC,LCT,N,X,Y,NROW,NT,LTRI,LOUT,PRNTX,ifortran)
C
...
C
    ...
    1   CONTINUE
     ...
     .             X,Y,LIST,LPTR,LEND,TITLE,NUMBR, IER,ifortran+2)
      IF (IER .EQ. 0) THEN
...
!     .             'successfully created.'/)
      END
      SUBROUTINE TRMTST_ext (N,X,Y,LIST,LPTR,LEND,LNEW,TOL,
     .                   LUN, ARMAX,IER)
      dll_export TRMTST_ext
      INTEGER N, LIST(*), LPTR(*), LEND(N), LNEW, LUN, IER
      REAL    X(N), Y(N), TOL, ARMAX
      callTRMTST (N,X,Y,LIST,LPTR,LEND,LNEW,TOL,
     .                   LUN, ARMAX,IER,2)
      return
      end
...
!      IF (RITE) WRITE (LUN,150) NFAIL
      IF (RITE) CALL Trmtst150(IFORTRAN,  NFAIL)
