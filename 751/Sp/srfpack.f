      SUBROUTINE ARCINT_ext (B,X1,X2,Y1,Y2,H1,H2,HX1,HX2,HY1,
     .                   HY2,SIGMA,DFLAG, HP,HXP,HYP,IER)
      dll_export ARCINT_ext
      INTEGER IER
      LOGICAL DFLAG
      REAL    B, X1, X2, Y1, Y2, H1, H2, HX1, HX2, HY1,
     .        HY2, SIGMA, HP, HXP, HYP
      call ARCINT (B,X1,X2,Y1,Y2,H1,H2,HX1,HX2,HY1,
     .                   HY2,SIGMA,DFLAG, HP,HXP,HYP,IER)
      return
      end
...
C
    1 IER = -1
      RETURN
      END
      SUBROUTINE CNTOUR_exp (NX,NY,X,Y,Z,CVAL,LC,NCMAX,IWK, XC,
     .                   YC,ILC,NC,IER)
      dll_export CNTOUR_exp
      INTEGER NX, NY, LC, NCMAX, IWK(NX,*), ILC(NCMAX), NC,
     .        IER
      REAL    X(NX), Y(NY), Z(NX,NY), CVAL, XC(LC), YC(LC)
      call CNTOUR (NX,NY,X,Y,Z,CVAL,LC,NCMAX,IWK, XC,
     .                   YC,ILC,NC,IER,4)
      RETURN
      END
...
C
   16 IF (LCON .GT. LMX) IER = LCON
      ...
      RETURN
      END
      SUBROUTINE COORDS_ext (XP,YP,X1,X2,X3,Y1,Y2,Y3, B1,B2,
     .                   B3, IER)
      dll_export COORDS_ext
      INTEGER IER
      REAL    XP, YP, X1, X2, X3, Y1, Y2, Y3, B1, B2, B3
      call COORDS (XP,YP,X1,X2,X3,Y1,Y2,Y3, B1,B2,
     .                   B3, IER)
      RETURN
      END
...
C
    1 IER = -1
      RETURN
      END
      SUBROUTINE CRPLOT_ext (LUN,PLTSIZ,NX,NY,PX,PY,PZ,NCON,IWK,
     .                   XC,YC, IER)
      dll_export CRPLOT_ext
      INTEGER LUN, NX, NY, NCON, IWK(*), IER
      REAL    PLTSIZ, PX(NX), PY(NY), PZ(NX,NY),
     .        XC(*), YC(*)
      call CRPLOT (LUN,PLTSIZ,NX,NY,PX,PY,PZ,NCON,IWK,
     .                   XC,YC, IER,4)
      RETURN
      END
...
C
   15 IER = 5
      RETURN
      END
      SUBROUTINE FVAL_ext (XP,YP,X1,X2,X3,Y1,Y2,Y3,F1,F2,F3,
     ...
     .        SIG2, SIG3, FP
      call FVAL (XP,YP,X1,X2,X3,Y1,Y2,Y3,F1,F2,F3,
     .                 FX1,FX2,FX3,FY1,FY2,FY3,SIG1,SIG2,
     .                 SIG3, FP,IER)
      RETURN
      END
...
C
      ...
      RETURN
      END
      SUBROUTINE GETSIG_ext (N,X,Y,H,LIST,LPTR,LEND,HXHY,
     .                   TOL, SIGMA, DSMAX,IER)
      dll_export GETSIG_ext
      INTEGER N, LIST(*), LPTR(*), LEND(N), IER
      REAL    X(N), Y(N), H(N), HXHY(2,N), TOL, SIGMA(*),
     .        DSMAX
      call GETSIG (N,X,Y,H,LIST,LPTR,LEND,HXHY,
     .                   TOL, SIGMA, DSMAX,IER,2)
      RETURN
      END
...
C
   12 DSMAX = DSM
      ...
      RETURN
      END
      SUBROUTINE GIVENS_ext ( A,B, C,S)
      dll_export GIVENS_ext
      REAL A, B, C, S
      call GIVENS ( A,B, C,S)
      RETURN
      END
...
C
   14 IER = -2
      RETURN
      END
      SUBROUTINE GRADG_ext (NCC,LCC,N,X,Y,Z,LIST,LPTR,LEND,
     .                  IFLGS,SIGMA, NIT,DGMAX,GRAD, IER)
      dll_export GRADG_ext
...
C
   11 NIT = 0
      ...
      RETURN
      END
      SUBROUTINE GRADL_ext (K,NCC,LCC,N,X,Y,Z,LIST,LPTR,
     .                  LEND, DX,DY,IER)
      dll_export GRADL_ext
      INTEGER K, NCC, LCC(*), N, LIST(*), LPTR(*),
     .        LEND(N), IER
...
C
   14 IER = -2
      RETURN
      END
      SUBROUTINE GRCOEF_ext (SIGMA,DCUB, D,SD)
      dll_export GRCOEF_ext
      REAL SIGMA, DCUB, D, SD
      call GRCOEF (SIGMA,DCUB, D,SD)
      RETURN
      END
...
C
      ...
      RETURN
      END
      SUBROUTINE INTRC0_ext (PX,PY,NCC,LCC,N,X,Y,Z,LIST,LPTR,
     .                   LEND, IST, PZ,IER)
      dll_export INTRC0_ext
      INTEGER NCC, LCC(*), N, LIST(*), LPTR(*), LEND(N),
     .        IST, IER
      REAL    PX, PY, X(N), Y(N), Z(N), PZ
      call INTRC0 (PX,PY,NCC,LCC,N,X,Y,Z,LIST,LPTR,
     .                   LEND, IST, PZ,IER)
      RETURN
      END
...
C
      ...
      GO TO 2
      END
      SUBROUTINE INTRC1_ext (PX,PY,NCC,LCC,N,X,Y,Z,LIST,LPTR,
     .                   LEND,IFLGS,SIGMA,GRAD,
     .                   DFLAG, IST, PZ,PZX,PZY,IER)
      dll_export INTRC1_ext
      INTEGER NCC, LCC(*), N, LIST(*), LPTR(*), LEND(N),
     .        IFLGS, IST, IER
      LOGICAL DFLAG
      REAL    PX, PY, X(N), Y(N), Z(N), SIGMA(*), GRAD(2,N),
     .        PZ, PZX, PZY
      call INTRC1 (PX,PY,NCC,LCC,N,X,Y,Z,LIST,LPTR,
     .                   LEND,IFLGS,SIGMA,GRAD,
     .                   DFLAG, IST, PZ,PZX,PZY,IER)
      return
      END
...
C
     ...
     .         (ZY2-ZY1)*YQP)/D2
      ...
      RETURN
      END
      SUBROUTINE ROTATE_ext (N,C,S, X,Y )
      dll_export ROTATE_ext
      INTEGER N
      REAL    C, S, X(N), Y(N)
      call ROTATE (N,C,S, X,Y )
      RETURN
      END
...
C
    ...
    1   CONTINUE
      RETURN
      END
      SUBROUTINE SETRO1_ext (XK,YK,ZK,XI,YI,ZI,S1,S2,W, ROW)
      dll_export SETRO1_ext
      REAL XK, YK, ZK, XI, YI, ZI, S1, S2, W, ROW(6)
      call SETRO1 (XK,YK,ZK,XI,YI,ZI,S1,S2,W, ROW)
      RETURN
      END
...
C
      ...
      RETURN
      END
      SUBROUTINE SGPRNT_ext (N,LUNIT,LIST,LPTR,LEND,SIGMA)
      dll_export SGPRNT_ext
      INTEGER N, LUNIT, LIST(*), LPTR(*), LEND(N)
      REAL    SIGMA(*)
      call SGPRNT (N,LUNIT,LIST,LPTR,LEND,SIGMA,2)
      RETURN
      END
...
!     .        'NMAX = ',I4,' ***')
     ...
     .                    IFLGB,HBND,TOL,IFLGS, SIGMA, IER,ifortran)
      dll_import Sig0_100, Sig0_110, Sig0_120, Sig0_130, Sig0_140
...
!        IF (RF .LT. 0.) WRITE (LUN,100) N1, N2, BND
        IF (RF .LT. 0.) CALL Sig0_100(IFORTRAN, N1, N2, BND)
 !       IF (RF .GT. 0.) WRITE (LUN,110) N1, N2, BND
        IF (RF .GT. 0.) CALL Sig0_110(IFORTRAN, N1, N2, BND)
      ENDIF
...
!      IF (LUN .GE. 0) WRITE (LUN,120) SIG, SNEG, F0, FMAX
      IF (LUN .GE. 0) CALL Sig0_120(IFORTRAN, SIG, SNEG, F0, FMAX)
...
!      IF (LUN .GE. 0) WRITE (LUN,130) NIT, SIG, F
      IF (LUN .GE. 0) CALL Sig0_130(IFORTRAN, NIT, SIG, F)
...
!      IF (LUN .GE. 0) WRITE (LUN,140) DSIG
      IF (LUN .GE. 0) CALL Sig0_140(IFORTRAN, DSIG)
...
C
   11 SIG0 = -1.
     ...
     .                    IFLGB,HPBND,TOL,IFLGS, SIGMA, IER, ifortran)
      dll_import Sig1_100, Sig1_110, Sig1_120, Sig1_130, Sig1_140
      INTEGER N1, N2, N, LIST(*), LPTR(*), LEND(N), IFLGB,
     .        IFLGS, IER
...
C
      IF (LUN .GE. 0) THEN
!        IF (RF .LT. 0.) WRITE (LUN,100) N1, N2, BND
        IF (RF .LT. 0.) CALL Sig1_100(IFORTRAN, N1, N2, BND)
!        IF (RF .GT. 0.) WRITE (LUN,110) N1, N2, BND
        IF (RF .GT. 0.) CALL Sig1_110(IFORTRAN, N1, N2, BND)
      ENDIF
...
!      IF (LUN .GE. 0) WRITE (LUN,120) F0, FMAX, SIG
      IF (LUN .GE. 0) CALL Sig1_120(IFORTRAN, F0, FMAX, SIG)
...
C
      NIT = NIT + 1
!      IF (LUN .GE. 0) WRITE (LUN,130) NIT, SIG, F
      IF (LUN .GE. 0) CALL Sig1_130(IFORTRAN, NIT, SIG, F)
!  130 FORMAT (1X,3X,I2,' -- SIG = ',E15.8,', F = ',
!     .        E15.8)
      IF (F0*F .LT. 0.) THEN
C
...
C
    8 DSIG = -F*DSIG/(F-F0)
!      IF (LUN .GE. 0) WRITE (LUN,140) DSIG
      IF (LUN .GE. 0) CALL Sig1_140(IFORTRAN, DSIG)
!  140 FORMAT (1X,8X,'DSIG = ',E15.8)
      IF ( ABS(DSIG) .GT. ABS(DMAX)  .OR.
     .     DSIG*DMAX .GT. 0. ) GO TO 7
C
...
C
   10 SIG1 = -1.
     ...
     .                    TOL,IFLGS, SIGMA, IER,ifortran)
      dll_import Sig2_100, Sig2_110
...
!      IF (LUN .GE. 0) WRITE (LUN,100) N1, N2
      IF (LUN .GE. 0) CALL Sig2_100(IFORTRAN,  N1, N2)
...
!      IF (LUN .GE. 0) WRITE (LUN,110) NIT, SIG, F, FP
      IF (LUN .GE. 0) CALL Sig2_110(IFORTRAN, NIT, SIG, F, FP)
...
C
   10 NIT = 0
     ...
     .                   FXFY,IER,ifortran)
      dll_import Smsurf100, Smsurf110, Smsurf120, Smsurf130
      INTEGER NCC, LCC(*), N, LIST(*), LPTR(*), LEND(N),
     .        IFLGS, IER
...
!        IF (LUN .GE. 0) WRITE (LUN,100)
        IF (LUN .GE. 0) CALL Smsurf100(IFORTRAN)
...
!      IF (LUN .GE. 0) WRITE (LUN,110) SM, TOL, NITMAX, G0
      IF (LUN .GE. 0) CALL Smsurf110(IFORTRAN, SM, TOL, NITMAX, G0)
...
!      IF (LUN .GE. 0) WRITE (LUN,120) ITER, P, G, NIT, DFMAX
      IF (LUN .GE. 0) CALL Smsurf120(IFORTRAN, ITER, P, G, NIT, DFMAX)
...
!      IF (LUN .GE. 0) WRITE (LUN,130) DP
      IF (LUN .GE. 0) CALL Smsurf130(IFORTRAN, DP)
...
C
      ...
      GO TO 4
      END
      SUBROUTINE SNHCSH_ext (X, SINHM,COSHM,COSHMM)
      dll_export SNHCSH_ext
      REAL X, SINHM, COSHM, COSHMM
      call SNHCSH (X, SINHM,COSHM,COSHMM)
      return
      END
...
C
      ...
      RETURN
      END
      REAL FUNCTION TRVOL_ext (X1,X2,X3,Y1,Y2,Y3,Z1,Z2,Z3)
      dll_export TRVOL_ext
      REAL X1, X2, X3, Y1, Y2, Y3, Z1, Z2, Z3
      TRVOL_ext = trvol(X1,X2,X3,Y1,Y2,Y3,Z1,Z2,Z3)
      RETURN
      END
...
C
      ...
      RETURN
      END
      SUBROUTINE TVAL_ext (X,Y,X1,X2,X3,Y1,Y2,Y3,Z1,Z2,Z3,ZX1,
     ...
     .        ZX1, ZX2, ZX3, ZY1, ZY2, ZY3, F, FX, FY
      call TVAL (X,Y,X1,X2,X3,Y1,Y2,Y3,Z1,Z2,Z3,ZX1,
     .                 ZX2,ZX3,ZY1,ZY2,ZY3,DFLAG, F,FX,FY,
     .                 IER)
      RETURN
      END
...
C
    9 IER = 1
      ...
      RETURN
      END
      SUBROUTINE UNIF_ext (NCC,LCC,N,X,Y,Z,GRAD,LIST,LPTR,LEND,
...
C
    4 IER = -2
      RETURN
      END
      REAL FUNCTION VOLUME_ext (NCC,LCC,N,X,Y,Z,LIST,LPTR,LEND)
      dll_export VOLUME_ext
      INTEGER NCC, LCC(*), N, LIST(*), LPTR(*), LEND(N)
      REAL    X(N), Y(N), Z(N)
      VOLUME_ext = VOLUME (NCC,LCC,N,X,Y,Z,LIST,LPTR,LEND)
      RETURN
      END
...
C
    5 VOLUME = 0.
      RETURN
      END
      SUBROUTINE ZGRADG_ext (NCC,LCC,N,X,Y,LIST,LPTR,LEND,
     .                   IFLGS,SIGMA, NIT,DZMAX,Z,
     .                   GRAD, IER)
      dll_export ZGRADG_ext
      INTEGER NCC, LCC(*), N, LIST(*), LPTR(*), LEND(N),
     .        IFLGS, NIT, IER
...
C
   14 NDV = 0
      ...
      RETURN
      END
      SUBROUTINE ZINIT_ext (NCC,LCC,N,X,Y,LIST,LPTR,
     .                  LEND, Z, IER)
      dll_export ZINIT_ext
      INTEGER NCC, LCC(*), N, LIST(*), LPTR(*), LEND(N),
     .        IER
      REAL    X(N), Y(N), Z(N)
      call ZINIT (NCC,LCC,N,X,Y,LIST,LPTR,
     .                  LEND, Z, IER)
      RETURN
      END
